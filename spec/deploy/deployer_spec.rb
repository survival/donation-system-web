...
module Deploy
  RSpec.describe Deployer do
    ...
    before do
      allow_any_instance_of(FileHelper).to receive(:append_digest)
        .and_return([])
      ...
      allow_any_instance_of(FileHelper).to receive(:undo_append_digest)
        .and_return([])
    ...
    describe 'when successful' do
      ...
      it 'appends digest to file path' do
        expect_any_instance_of(FileHelper).to receive(:append_digest)
        deployer.run
      end

      it 'uploads digested file to Amazon' do
      ...
      it 'undoes digesting' do
        expect_any_instance_of(FileHelper).to receive(:undo_append_digest)
        deployer.run
      end
    ...
    describe 'when digesting fails' do
      before do
        allow_any_instance_of(FileHelper).to receive(:append_digest)
          .and_return(['error'])
      end
    ...
    describe 'when removing digested file fails' do
      it 'has errors' do
        allow_any_instance_of(FileHelper).to receive(:undo_append_digest)
          .and_return(['error'])
        expect(deployer.run).to include('error')
