...
module Deploy
  class Deployer
    ...
    def append_digest
      errors << file_helper.append_digest
      errors.flatten!
    end
    ...
    def undo_append_digest
      errors << file_helper.undo_append_digest
      errors.flatten!
    end
    ...
    def amazon_file_uploader
      AmazonFileUploader.new(
        ...
        bucket_name: bucket_name,
        local_filepath: file_helper.local_digested_filepath,
        upstream_filepath: file_helper.upstream_digested_filepath
      )
    end
    ...
    def env_var
      {
        heroku_setup[:asset_path_envname] =>
          file_helper.upstream_digested_filepath
      }
    end
